...
export const BOARD_MEETING_ROLES = ['admin', 'supervisor', 'ceo'];
export const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{12}$/i;

export async function requireBoardMeetingUser(request: Request, allowedRoles = BOARD_MEETING_ROLES) {
  ...
  if (!allowedRoles.includes(user.role)) {
    return { error: NextResponse.json({ error: 'Board meeting access required' }, { status: 403 }) };
  }
