...
import { nullableDateOrTime, nullableString, requireBoardMeetingUser, stringValue } from './_auth';
...
const confidentialityValues = ['open', 'internal', 'board-restricted', 'confidential'];
...
export async function POST(request: Request) {
  ...
  const confidentiality = stringValue(form.confidentiality) || 'board-restricted';
  ...
  const payload = {
    ...
    meeting_type: stringValue(form.meeting_type) || 'Board Meeting',
    title,
  ...
  let meetingResult = meetingId
    ? await auth.supabase.from('board_meetings').update(payload).eq('id', meetingId).select().single()
    : await auth.supabase.from('board_meetings').insert([payload]).select().single();
  ...
  if (meetingResult.error?.message.includes('minutes_updated')) {
    ...
    meetingResult = meetingId
      ? await auth.supabase.from('board_meetings').update(legacyPayload).eq('id', meetingId).select().single()
      : await auth.supabase.from('board_meetings').insert([legacyPayload]).select().single();
  }
  ...
  const { error: participantDeleteError } = await auth.supabase
    .from('board_meeting_participants')
    .delete()
  ...
  if (cleanParticipants.length) {
    const { error } = await auth.supabase.from('board_meeting_participants').insert(cleanParticipants);
  ...
  if (cleanAgenda.length) {
    const { error } = await auth.supabase.from('board_meeting_agenda_items').insert(cleanAgenda);
