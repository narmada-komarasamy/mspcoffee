...
type MeetingFile = {
  id: string;
  meeting_id: string;
  file_type: "board-pack" | "audio" | "minutes-draft" | "signed-minutes" | "attachment";
  file_name: string;
  file_path: string;
...
type Meeting = {
  ...
  confidentiality: "open" | "internal" | "board-restricted" | "confidential";
  ...
  created_at: string;
  updated_at: string;
  board_meeting_participants?: Participant[];
  board_meeting_agenda_items?: AgendaItem[];
  board_meeting_actions?: MeetingAction[];
  board_meeting_files?: MeetingFile[];
};
...
const blankForm = (userName = ""): MeetingForm => ({
  ...
  confidentiality: "board-restricted",
  agenda_summary: "",
  minutes_draft: "",
  decisions: "",
  approval_status: "draft",
  minute_owner: userName || "Company Secretary",
  reviewer: "CEO",
  approver: "Chairman",
  approval_date: null,
...
export default function BoardMeetingsPage() {
  ...
  const loadMeetings = useCallback(async () => {
    ...
    if (error) {
      setMessage({ type: "error", text: `Failed to load board meetings: ${error.message}` });
      setLoading(false);
      return;
    }
    ...
    const rows = (data ?? []).map((m) => ({
      ...m,
      board_meeting_agenda_items: [...(m.board_meeting_agenda_items ?? [])].sort((a, b) => a.item_no - b.item_no),
      board_meeting_actions: [...(m.board_meeting_actions ?? [])].sort((a, b) => (a.due_date ?? "").localeCompare(b.due_date ?? "")),
      board_meeting_files: [...(m.board_meeting_files ?? [])].sort((a, b) => b.created_at.localeCompare(a.created_at)),
    })) as Meeting[];
  ...
  useEffect(() => {
    ...
    setParticipants(selected.board_meeting_participants?.length ? selected.board_meeting_participants : blankParticipants());
    setAgenda(selected.board_meeting_agenda_items?.length ? selected.board_meeting_agenda_items : blankAgenda());
  }, [selected]);

  const stats = useMemo(() => {
    const actions = meetings.flatMap((m) => m.board_meeting_actions ?? []);
  ...
  const filteredMeetings = useMemo(() => {
    ...
    return meetings.filter((m) => {
      const matchesSearch = !q || [
        ...
        m.location ?? "",
        m.agenda_summary ?? "",
        ...(m.board_meeting_participants ?? []).map((p) => p.name),
  ...
  const saveMeeting = async () => {
    ...
    await loadMeetings();
    setSaving(false);
    setMessage({ type: "ok", text: "Board meeting saved." });
  };
    ...
    <div className={css.page}>
      <section className={css.hero}>
        <div>
          ...
          <h1 className={css.heroTitle}>Board Meetings</h1>
      ...
      <section className={css.stats}>
        <div className={css.stat}>
          ...
          <div className={css.statSub}>Board and management records</div>
        </div>
        <div className={css.stat}>
              ...
              <div className={css.formGrid}>
                ...
                <div className={css.field}>
                  ...
                  <select className={css.select} value={form.confidentiality} onChange={(e) => updateForm("confidentiality", e.target.value as Meeting["confidentiality"])}>
                    <option value="open">Open</option><option value="internal">Internal</option><option value="board-restricted">Board restricted</option><option value="confidential">Confidential</option>
                  </select>
                </div>
              </div>
            </div>
        ...
        <div className={css.panel}>
          <div className={css.panelHead}>
            <div>
              ...
              <div className={css.panelSub}>Board pack, audio, draft minutes, signed minutes and attachments</div>
            </div>
          </div>
          <div className={css.detailBody}>
            <div className={css.fileGrid}>
              {([
                ["board-pack", "Board Pack", FileText],
